...
# ── Tool argument summarization (for logging) ─────────────────────────

# NOTE: ``ModelConfig`` and ``MultiModelClient`` have been moved to
# ``src.model_routing`` to follow the Single Responsibility Principle.
